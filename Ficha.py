...
# pipreqs --encoding utf-8 "D:\Dropbox\Empresa\Buydepa\PROYECTOS\APPRAISAL\streamlit\ficha"
...
@st.experimental_memo
def getdata(code,pais,tiponegocio,tipoinmueble):
    tabla  = f'{pais.lower()}_{tiponegocio.lower()}_{tipoinmueble.lower()}_market'
    engine = create_engine(f'mysql+mysqlconnector://{user}:{password}@{host}/{schema}')
    return pd.read_sql_query(f"""SELECT pais, zona1, zona2, zona3, zona4, direccion, latitud, longitud, estrato,areaconstruida,habitaciones,banos,garajes,valorventa,valorarriendo,antiguedad,descripcion,url, img1, img2, img3, img4, img5, img6, img7, img8, img9, img10, img11, img12, img13, img14, img15 FROM appraisal.{tabla} WHERE id='{code}'""", engine)
...
if st.session_state.data_ficha.empty:
    ...
    if code!='' and pais!='' and any([x for x in ['apartamento','casa'] if x in tipoinmueble.lower()]) and  any([x for x in ['venta','arriendo'] if x in tiponegocio.lower()]):
        st.session_state.data_ficha = getdata(code,pais,tiponegocio,tipoinmueble)
        if st.session_state.data_ficha.empty:
    ...
    with col2:
        ...
        outuput_list = {'Ciudad:':None,'Dirección:':None,'Localidad:':None,'Barrio:':None,'Tipo de inmueble:':None,'Estrato:':None,'Antiguedad:':None}
